...
#include "../lib/IR/SymbolTableListTraitsImpl.h"
...
PreservedAnalyses GlobalDCEPass::run(Module &M, ModuleAnalysisManager &MAM) {
  bool Changed = false;
  if (M.CachingMode) {
    // Everything sharing same comdat must be made alive, even if it's
    // not alive now and thus will not found in for (Function &F : M).
    for (auto &I : M.getValueSymbolTable())
      if (GlobalValue *GV = dyn_cast<GlobalValue>(I.second))
        if (Comdat *C = GV->getComdat())
