...
public:
  ...
  bool erase(StructType *STy) {
    return LayoutInfo.erase(STy);
  }

  unsigned count(StructType *STy) const {
    return LayoutInfo.count(STy);
  }
};
...
bool DataLayout::erase(StructType *Ty) const {
  if (!LayoutMap)
    return true;
  StructLayoutMap *STM = static_cast<StructLayoutMap *>(LayoutMap);
  return STM->erase(Ty);
...
unsigned DataLayout::count(StructType *Ty) const {
  if (!LayoutMap)
    return true;
  StructLayoutMap *STM = static_cast<StructLayoutMap *>(LayoutMap);
  return STM->count(Ty);
