...
namespace {
class invalidateFunctionUsers {
  unsigned StackDepth;

public:
  invalidateFunctionUsers(Constant *C) : StackDepth(0) {
    find(C);
  }
  void find(Constant *C) {
    // 3 is enough to pass zapcc tests and libs, 2 too small.
    if (StackDepth == 3)
      return;
    ++StackDepth;
    for (User *U : C->users()) {
...
void Value::doRAUW(Value *New, bool NoMetadata) {
  ...
  if (GlobalValue *GV = dyn_cast<GlobalValue>(this))
    if (Module *M = GV->getParent())
      if (M->CachingMode)
        invalidateFunctionUsers IFU(GV);
