...
void DIBuilder::finalize() {
  ...
  if (!RetainValues.empty() || CUNode->getRetainedTypes().size())
  ...
  if (!AllGVs.empty() || CUNode->getGlobalVariables().size())
    CUNode->replaceGlobalVariables(MDTuple::get(VMContext, AllGVs));

  if (!AllImportedModules.empty() || CUNode->getImportedEntities().size())
