...
GlobalValue *Module::getNamedValue(StringRef Name) const {
  // zapcc: do not setAlive here as callers such as
  // getGlobalVariable(StringRef Name, bool AllowLocal)
  // do not always want to make it really alive.
...
Function *Module::getFunction(StringRef Name) const {
  Function *F = dyn_cast_or_null<Function>(getNamedValue(Name));
  if (F)
    F->setAliveDefinition();
  return F;
}
...
GlobalVariable *Module::getGlobalVariable(StringRef Name,
  ...
  if (GlobalVariable *Result =
    ...
    {
      if (!Result->isAliveDeclaration())
        Result->setAliveDefinition();
      return Result;
    }
  return nullptr;
}
...
GlobalAlias *Module::getNamedAlias(StringRef Name) const {
  GlobalAlias *GA = dyn_cast_or_null<GlobalAlias>(getNamedValue(Name));
  if (GA)
    GA->setAliveDefinition();
  return GA;
}
...
bool Module::isAlive(const DINode *DIN) const {
  if (!CachingMode)
    return true;
  return AliveDIs.count(DIN);
}

void Module::setAlive() const {
  CachingMode = false;
  ...
  for (GlobalAlias &GA : aliases())
    GA.setAliveDefinition();
  for (GlobalIFunc &GIS : ifuncs())
    GIS.setAliveDefinition();
  CachingMode = true;
}

void Module::resetAlive() const {
  CachingMode = false;
  for (Function &F : functions())
    F.resetAlive();
  for (GlobalVariable &GV : globals())
...
void Module::printSymbolTablesize(StringRef Prefix) const {
  llvm::errs() << Prefix << " #symbols = " << getValueSymbolTable().size()
               << "\n";
}
