...
public:
  ...
  /// run - Execute all of the passes scheduled for execution.  Keep track of
  /// whether any of the passes modifies the module, and if so, return true.
  bool runOnModule(Module &M, runKind Kind = RK_All);
...
public:
  ...
  /// run - Execute all of the passes scheduled for execution.  Keep track of
  /// whether any of the passes modifies the module, and if so, return true.
  bool run(Module &M, runKind Kind = RK_All);
...
bool FPPassManager::runOnFunction(Function &F) {
  ...
  for (unsigned Index = 0; Index < getNumContainedPasses(); ++Index) {
    FunctionPass *FP = getContainedPass(Index);
    if (FP->getPassKind() != PassKind::PT_Function)
      continue;
    bool LocalChanged = false;
...
bool
MPPassManager::runOnModule(Module &M, runKind Kind) {
  bool Changed = false;
  if (Kind != RK_Finalization) {
...
/// run - Execute all of the passes scheduled for execution.  Keep track of
/// whether any of the passes modifies the module, and if so, return true.
bool PassManagerImpl::run(Module &M, runKind Kind) {
  bool Changed = false;
  if (Kind != RK_Finalization) {
  clearInversedLastUser();
  TimingInfo::createTheTimeInfo();
  ...
  initializeAllAnalysisInfo();
  for (unsigned Index = 0; Index < getNumContainedManagers(); ++Index) {
    Changed |= getContainedManager(Index)->runOnModule(M, RK_Initialization);
    M.getContext().yield();
  }
  }
  ...
  if (Kind != RK_Initialization) {
  for (unsigned Index = 0; Index < getNumContainedManagers(); ++Index)
    Changed |= getContainedManager(Index)->runOnModule(M, RK_Finalization);
  for (ImmutablePass *ImPass : getImmutablePasses())
    Changed |= ImPass->doFinalization(M);
  }
...
/// run - Execute all of the passes scheduled for execution.  Keep track of
/// whether any of the passes modifies the module, and if so, return true.
bool PassManager::run(Module &M, runKind Kind) {
  return PM->run(M, Kind);
}

Pass *PassManager::findPass(AnalysisID AID) {
  return (static_cast<PMTopLevelManager *>(PM))->findPass(AID);
}
...
void TimingInfo::createTheTimeInfo() {
  if (TimePassesIsEnabled == static_cast<bool>(TheTimeInfo))
    return;
