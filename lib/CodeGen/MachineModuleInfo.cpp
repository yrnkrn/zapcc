...
MachineFunction *MachineModuleInfo::get(const Function *F) {
  const auto &I = MachineFunctions.find(F);
  if (I == MachineFunctions.end())
    return nullptr;
  MachineFunction *MF = I->second.get();
  // Cleanup MF for reuse.
  for (MachineBasicBlock &MBB : *MF) {
    MBB.resetSymbolRaw();
    for (MachineInstr &MI : MBB)
      for (MachineOperand &Op : MI.operands())
        if (Op.isMCSymbol())
          Op.getMCSymbol()->setUndefined();
  }
  return MF;
}

bool MachineModuleInfo::erase(const Function *F) {
  Personalities.erase(
      std::remove(Personalities.begin(), Personalities.end(), F),
      Personalities.end());
  const auto &I = MachineFunctions.find(F);
  if (I == MachineFunctions.end())
    return false;
  MachineFunction *MF = I->second.get();
  // release MF MCSymbols.
  for (MachineBasicBlock &MBB : *MF) {
    if (MCSymbol *Sym = MBB.getSymbolRaw())
      Sym->release();
    for (MachineInstr &MI : MBB)
      for (MachineOperand &Op : MI.operands())
        if (Op.isMCSymbol())
          Op.getMCSymbol()->release();
  }
  MachineFunctions.erase(I);
  LastRequest = nullptr;
  LastResult = nullptr;
  return true;
}
