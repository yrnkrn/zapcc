...
void MCContext::cleanup() {
  if (!CachingMode)
    return;

  // MCSymbol stores pointer to its name entry in Symbols so we can not move it.
  ...
  for (auto I = LocalSymbols.begin(), E = LocalSymbols.end(); I != E;) {
    auto J = I;
    ++J;
    if (!I->second->isAlive())
      LocalSymbols.erase(I);
    I = J;
  }

  for (auto I = UsedNames.begin(), E = UsedNames.end(); I != E;) {
    auto J = I;
    ++J;
    if (!Symbols.count(I->first()))
      UsedNames.erase(I);
    I = J;
  }
}
...
void MCContext::reset() {
  ...
  if (!CachingMode) {
  // UsedNames and Symbols use Allocator and must be cleared first.
