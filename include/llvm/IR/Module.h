...
public:
  template <typename T>
  class alive_iterator : public SymbolTableList<T>::iterator {
    SymbolTableList<T> *Self;

  public:
    alive_iterator(typename SymbolTableList<T>::iterator I,
                   SymbolTableList<T> *Self)
        : SymbolTableList<T>::iterator(I), Self(Self) {}
    // Support ->getIterator() usage in GlobalOpt.cpp.
    alive_iterator(typename SymbolTableList<T>::iterator I)
        : SymbolTableList<T>::iterator(I) {
      Module *M = (*this)->getParent();
      Self = &(M->*Module::getSublistAccess((T *)(nullptr)));
      // Self = SymbolTableListTraits<T>::getList(M);
    }
    // Constructor to support direct iterator construction in IRObject.cpp.
    alive_iterator(const T *NP)
        : SymbolTableList<T>::iterator(const_cast<T *>(NP)) {
      Module *M = (*this)->getParent();
      Self = &(M->*Module::getSublistAccess((T *)(nullptr)));
// Self = SymbolTableListTraits<T>::getList(M);
#if 0
      ilist_iterator<T> E = Self->end();
      while (*this != E && !(*this)->getAlive())
        ilist_iterator<T>::operator++();
#endif
    }
    alive_iterator &operator++() {
      ...
      if (M->CachingMode) {
        auto E = Self->end();
        do {
          SymbolTableList<T>::iterator::operator++();
        } while (*this != E && !(*this)->isAlive());
      } else {
        SymbolTableList<T>::iterator::operator++();
      }
      return *this;
  ...
  template <typename T> class alive_list : public SymbolTableList<T> {
  public:
    typedef alive_iterator<T> iterator;
    typedef alive_iterator<T> const_iterator;
    typedef std::reverse_iterator<iterator> reverse_iterator;
    typedef std::reverse_iterator<const_iterator> const_reverse_iterator;

    iterator begin() {
      auto I = SymbolTableList<T>::begin();
      auto E = SymbolTableList<T>::end();
      if (I != E && I->getParent()->CachingMode)
        while (I != E && !I->isAlive())
          ++I;
      return iterator(I, this);
    }
    const_iterator begin() const {
      return const_cast<alive_list<T> *>(this)->begin();
    }
    iterator end() {
#if 0
      auto B = iplist<T>::begin();
      auto I = iplist<T>::end();
      while (I != B) {
        auto J = I;
        --J;
      ...
      return iterator(I, this);
#endif
      return iterator(SymbolTableList<T>::end(), this);
    }
    const_iterator end() const {
      return const_cast<alive_list<T> *>(this)->end();
    }
  ...
  using GlobalListType = alive_list<GlobalVariable>;
  using FunctionListType = alive_list<Function>;
  using AliasListType = alive_list<GlobalAlias>;
  using IFuncListType = alive_list<GlobalIFunc>;
#if 0
...
/// @{
  mutable bool CachingMode;
  std::function<bool(const Function *)> invalidateMachineFunctionHandler;
  // Live DebugInfo computed by AliveFinder.
  SmallPtrSet<const DINode *, 1> AliveDIs;
  bool isAlive(const DINode *DIN) const;
  void setAlive() const;
  void resetAlive() const;
  void printSymbolTablesize(StringRef Prefix) const;
