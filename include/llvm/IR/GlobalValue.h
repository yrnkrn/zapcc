...
protected:
  ...
  mutable PointerIntPair<const void *, 2> DeclAndAlive;
...
public:
  ...
  enum AliveTypes { Dead = 0, Declaration = 1, Definition = 2 };
  bool isAliveDeclaration() const {
    return DeclAndAlive.getInt() & AliveTypes::Declaration;
  }
  bool isAliveDefinition() const {
    return DeclAndAlive.getInt() & AliveTypes::Definition;
  }
  bool isAlive() const { return DeclAndAlive.getInt() != AliveTypes::Dead; }
  void setAliveDeclaration() const {
    DeclAndAlive.setInt(AliveTypes::Declaration);
  }
  void setAliveDefinition() const {
    DeclAndAlive.setInt(AliveTypes::Definition);
  }
  void resetAlive() const { DeclAndAlive.setInt(AliveTypes::Dead); }
  void setDecl(const void *D) const { DeclAndAlive.setPointer(D); }
  const void *getDecl() const { return DeclAndAlive.getPointer(); }
  static GlobalValue *fromValue(Value *V);
