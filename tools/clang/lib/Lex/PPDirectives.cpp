...
void Preprocessor::HandleUserDiagnosticDirective(Token &Tok,
  ...
  if (LangOpts.CachingMode)
    if (Msg.find("included multiple times") != StringRef::npos ||
        Msg.find("included once") != StringRef::npos)
      return;
...
void Preprocessor::HandleIncludeDirective(SourceLocation HashLoc,
  ...
  if (LangOpts.CachingMode) {
    ...
    if (Filename.find("/preprocessed/") != StringRef::npos)
      if (SourceMgr.hasFileInfo(File))
        ShouldEnter = false;
    if (const FileEntry *FE = CurPPLexer->getFileEntry()) {
      NewFEs.push_back({ File, FE });
      IncludeMgr.include(File, FE);
    }
    IncludeMgr.include(File);
  }
    ...
    if (ImmediatelyAfterHeaderGuard) {
      ...
      if (LangOpts.CachingMode) {
        const IdentifierInfo *ControllingMacro =
            CurPPLexer->MIOpt.GetControllingMacro();
        // FIXME: needed?
        if (ControllingMacro == CurPPLexer->MIOpt.GetDefinedMacro()) {
          StringRef Name = ControllingMacro->getName();
          if (Name.endswith("_H") || Name.endswith("_H_") ||
              Name.endswith("_HH"))
            if (const FileEntry *FE = CurPPLexer->getFileEntry())
              HeaderInfo.getFileInfo(FE).isPragmaOnce = true;
        }
      }
    }
...
void Preprocessor::HandleIfdefDirective(Token &Result, bool isIfndef,
  ...
  } else if (LangOpts.CachingMode && isIfndef && MI && MI->UsedForHeaderGuard) {
    // We found #ifndef include guard used outside its original file (since it
    // won't be reincluded) with the macro set. For valid caching, make this
    // include file "include" the guarded include file.
    auto I = IncludeGuards.find(MII);
    assert(I != IncludeGuards.end());
    const FileEntry *Includee = I->second;
    IncludeMgr.include(Includee, CurPPLexer->getFileEntry());
  }
