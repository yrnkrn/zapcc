...
Preprocessor::Preprocessor(std::shared_ptr<PreprocessorOptions> PPOpts,
  ...
  InsertedIgnorePragmas = false;
  InsertedVisibleFEs = false;
...
void Preprocessor::CachingIncludeManager::include(const FileEntry *Includee,
                                                  const FileEntry *Includer) {
  assert(Includee->getName().size());
  assert(Includer);
  if (!PP.InsertedVisibleFEs) {
    for (const FileEntry *FE : PP.VisibleFEs)
      include(FE);
    PP.InsertedVisibleFEs = true;
  }
  IncludeesMap[Includer].push_back(Includee);
}

void Preprocessor::CachingIncludeManager::doCallbacks(
    const FileEntry *Includee) {
  auto I = LocForStartOfFile.find(Includee);
  if (I == LocForStartOfFile.end())
    return;
  SourceLocation SL = I->second;
  SrcMgr::CharacteristicKind Kind = PP.HeaderInfo.getFileDirFlavor(Includee);
  PP.Callbacks->FileChanged(SL, PPCallbacks::EnterFile, Kind);
  PP.Callbacks->FileChanged(SL, PPCallbacks::ExitFile, Kind);
}

void Preprocessor::CachingIncludeManager::include(const FileEntry *Includee) {
  if (!Includes.insert(Includee).second)
    return;
  if (PP.InsertedVisibleFEs) {
    if (PP.Callbacks)
      doCallbacks(Includee);
    PP.macroCheckChanged(Includee);
  }
  IncludedCallBack(Includee);
  auto Includees = IncludeesMap.find(Includee);
  if (Includees != IncludeesMap.end())
    for (const FileEntry *Included : Includees->second)
      include(Included);
}

bool Preprocessor::CachingIncludeManager::wasIncluded(SourceLocation SL) const {
  if (!PP.getLangOpts().CachingMode)
    return true;
  auto FileIDEntry = PP.getSourceManager().getFileIDEntry(SL);
  if (const FileEntry *FE = FileIDEntry.second)
    return Includes.count(FE);
  return true;
}

void Preprocessor::CachingIncludeManager::erase(const FileEntry *FE) {
  IncludeesMap.erase(FE);
  LocForStartOfFile.erase(FE);
}

static StringRef getMacroText(const SourceManager &SM, MacroInfo *MI) {
  if (!MI->getNumTokens())
    return "(defined)";
  SourceLocation SL = MI->tokens_begin()->getLocation();
  std::pair<FileID, unsigned> beginInfo = SM.getDecomposedLoc(SL);
  ...
  if (invalidTemp)
    return StringRef();
  unsigned DefinitionLength = MI->getDefinitionLength(SM);
  return file.substr(beginInfo.second, DefinitionLength);
}

// Can resue checkConfigMacro?
void Preprocessor::macroCheckChanged(const FileEntry *Includee) {
  auto I = FilesIdentifiersValues.find(Includee);
  if (I == FilesIdentifiersValues.end())
    return;
  for (auto Pair : I->second) {
    IdentifierInfo *II = Pair.first;
    StringRef OldValue = Pair.second;
    StringRef NewValue = CommandLineMainIdentifiersIdentifiers[II];
    if (OldValue != NewValue) {
      Diag(SourceLocation(), diag::err_pp_cached_macro_changed)
          << Includee->getName() << II->getName()
...
void Preprocessor::macroDefined(IdentifierInfo *II, MacroDirective *MD) {
  if (!CurPPLexer)
    return;
  FileID FID = CurPPLexer->getFileID();
  const FileEntry *FE = CurPPLexer->getFileEntry();
  if (FID == PredefinesFileID) {
    PresumedLoc PLoc = SourceMgr.getPresumedLoc(MD->getLocation());
    if (PLoc.isValid() && StringRef(PLoc.getFilename()) == "<command line>")
      GeneratedMacrosCommandLine.push_back({II, MD});
  } else if (FE) {
    StringRef Name = II->getName();
    if (Name.startswith("_") && Name.endswith_lower("_defined") &&
        HeaderInfo.getFileDirFlavor(FE) != SrcMgr::C_User)
      VisibleFEs.insert(FE);
    if (Name.startswith("__need_"))
      NeedMacros.push_back({II, MD});
    else
      GeneratedMacros[FE].push_back({II, MD});
  }
  if (MacroInfo *MI = MD->getMacroInfo()) {
    MI->setIsAllowRedefinitionsWithoutWarning(true);
    auto I = CommandLineMainIdentifiersIdentifiers.find(II);
    if (I != CommandLineMainIdentifiersIdentifiers.end())
      I->second = getMacroText(SourceMgr, MI);
  }
}

void Preprocessor::macroIdentifierUsed(IdentifierInfo *II) {
  if (!LangOpts.CachingMode)
    return;
  const PreprocessorLexer *Lexer = getCurrentFileLexer();
  if (!Lexer)
    return;
  auto I = CommandLineMainIdentifiersIdentifiers.find(II);
  if (I != CommandLineMainIdentifiersIdentifiers.end())
    if (const FileEntry *FE = Lexer->getFileEntry())
      FilesIdentifiersValues[FE][II] = I->second;
}

void Preprocessor::removeMacro(IdentifierInfo *II, MacroDirective *MD) {
  assert(II);
  assert(MD);
  assert(LangOpts.CachingMode);
  MacroState &StoredMD = CurSubmoduleState->Macros[II];
  MacroDirective *Latest = StoredMD.getLatest();
  MacroDirective *Prev = MD->getPrevious();
  if (MD == Latest) {
    // If last, set to previous if exists or remove completely if no previous.
    if (Prev)
      StoredMD.setLatest(Prev);
    else
      CurSubmoduleState->Macros.erase(II);
    II->setHasMacroDefinition(Prev && Prev->isDefined());
  } else {
    // Find who points at us.
    MacroDirective *Next = Latest;
    while (Next->getPrevious() != MD)
      Next = Next->getPrevious();
    // Patch it to point at our previous.
    Next->setPrevious(Prev);
  }
}

bool Preprocessor::wasIncluded(const IdentifierInfo *II,
                               MacroDirective *MD) const {
  return IncludeMgr.wasIncluded(MD->getLocation());
}

void Preprocessor::reset() {
  setExternalSource(nullptr);
  PTH.release();
  PragmaHandlersBackup.release();
  CommentHandlers.clear();
  IncrementalProcessing = false;
  CodeComplete = nullptr;
  CodeCompletionFile = nullptr;
  ...
  MacroExpandedTokens.clear();
  MacroExpandingLexersStack.clear();
  Record = nullptr;
  CachedTokens.clear();
  BacktrackPositions.clear();
  RegisterBuiltinMacros();
  IncludeMgr.Includes.clear();
  InsertedVisibleFEs = false;
}
...
void Preprocessor::EnterMainSourceFile() {
  ...
  if (!SourceMgr.isLoadedFileID(MainFileID)) {
    ...
    if (const FileEntry *FE = SourceMgr.getFileEntryForID(MainFileID))
    {
      ...
      if (LangOpts.CachingMode) {
        NewFEs.push_back({FE, FE});
        IncludeMgr.include(FE);
#ifndef NDEBUG
        ParsedFEs.insert(FE);
#endif
      }
