...
public:
  ...
  void eraseNamedDecl(const NamedDecl *ND) override {
    Uniquifier.erase(ND);
    SEHFilterIds.erase(ND);
    SEHFinallyIds.erase(ND);
  }
  void eraseCXXRecordDecl(const CXXRecordDecl *CXXRD) override {
    LambdaIds.erase(CXXRD);
  }
