...
void TemplateName::setTemplateDecl(TemplateDecl *TD) {
  if (Storage.is<TemplateDecl *>())
    Storage = TD;
  else if (QualifiedTemplateName *QTN = getAsQualifiedTemplateName())
    QTN->setTemplateDecl(TD);
  else if (SubstTemplateTemplateParmStorage *sub =
               getAsSubstTemplateTemplateParm())
    sub->getReplacement().setTemplateDecl(TD);
}
