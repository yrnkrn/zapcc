...
static void insertOverloadExprs(const ASTContext &C, OverloadExpr *OE) {
  if (!C.LangOpts.CachingMode)
    return;
  for (NamedDecl *ND : OE->decls())
    C.OverloadExprs[ND].push_back(OE);
}
...
bool OverloadExpr::erase(NamedDecl *ND) {
  DeclAccessPair *I = Results;
  DeclAccessPair *E = Results + NumResults;
  for (; I != E; ++I)
    if (*I == ND)
      break;
  if (I == E)
  ...
  --NumResults;
  if (NumResults)
    memmove(I, I + 1, (E - I - 1) * sizeof(DeclAccessPair));
  return true;
}
