...
void Decl::setLexicalDeclContext(DeclContext *DC) {
  ...
  insertModifiedDecls(dyn_cast<Decl>(getLexicalDeclContext()), this);
  if (isInSemaDC()) {
...
static const TranslationUnitDecl *getTranslationUnit(const Decl *D) {
  ...
  return cast_or_null<TranslationUnitDecl>(DC);
}

bool Decl::ignoreGeneratedDecl() const {
  // Maybe isa<FieldDecl>(this) ||
  return isa<TemplateTypeParmDecl>(this) || isa<ExternCContextDecl>(this) ||
         isa<NonTypeTemplateParmDecl>(this) || isa<ParmVarDecl>(this) ||
         isa<ImplicitParamDecl>(this) || isa<TranslationUnitDecl>(this);
}

void Decl::insertNewDecls(Decl *D) {
  if (D->ignoreGeneratedDecl())
    return;
  if (const TranslationUnitDecl *TU = getTranslationUnit(D)) {
    ASTContext &Ctx = TU->getASTContext();
    if (Ctx.getLangOpts().CachingMode) {
      Ctx.LiveDecls.insert(D);
      Ctx.NewDecls.push_back(D);
    }
  }
}

void Decl::insertModifiedDecls(Decl *Dependee, Decl *Dependent) {
  if (const TranslationUnitDecl *TU = getTranslationUnit(Dependent)) {
    ASTContext &Ctx = TU->getASTContext();
    if (Ctx.getLangOpts().CachingMode) {
      Ctx.LiveDecls.insert(Dependent);
      Ctx.ModifiedDecls.push_back({Dependee, Dependent});
    }
  }
}
