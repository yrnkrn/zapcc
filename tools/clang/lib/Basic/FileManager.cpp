...
void FileManager::invalidateCache(
  ...
  // The entry exist with different slashes or relative paths, must scan all.
  for (auto I = SeenFileEntries.begin(), E = SeenFileEntries.end(); I != E;) {
    auto J = I;
    ++J;
    if (FileEntries.count(I->getValue()))
      SeenFileEntries.erase(I);
    I = J;
  }
  for (const FileEntry *Entry : FileEntries) {
    if (FileSystemStatCache *Cache = StatCache.get())
      Cache->erase(Entry->getName());
// Must be before UniqueRealFiles.erase() which deallocates the FileEntry.
#ifndef NDEBUG
    FileEntry *FE = const_cast<FileEntry *>(Entry);
    FE->Name = "";
    FE->Dir = nullptr;
#endif
    UniqueRealFiles.erase(Entry->getUniqueID());
