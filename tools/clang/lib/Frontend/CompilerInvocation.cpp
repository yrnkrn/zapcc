...
void CompilerInvocationBase::resetOpts() {
  LangOpts->reset();
  // Do not reset TargetOpts as TargetInfo::CreateTargetInfo isn't called again.
  //*TargetOpts = TargetOptions();
  DiagnosticOpts->reset();
  PreprocessorOpts.reset(new PreprocessorOptions);
}
