...
std::unique_ptr<ASTConsumer> CompilerInstance::takeASTConsumer() {
  if (TheSema)
    TheSema->setASTConsumer(nullptr);
  return std::move(Consumer);
}

void CompilerInstance::setASTConsumer(std::unique_ptr<ASTConsumer> Value) {
  if (TheSema)
    TheSema->setASTConsumer(Value.get());
...
void CompilerInstance::createPreprocessor(TranslationUnitKind TUKind) {
  bool Full = !getLangOpts().CachingMode || !PP;
  ...
  if (PreprocessorCreatedCallBack)
    PreprocessorCreatedCallBack(*PP.get());
  } else { // Full
    PP->reset();
  }
...
bool CompilerInstance::ExecuteAction(FrontendAction &Act) {
  ...
  raw_ostream &OS = DiagsOS ? *DiagsOS : llvm::errs();

  // Create the target instance.
  if (!getLangOpts().CachingMode || !Target)
