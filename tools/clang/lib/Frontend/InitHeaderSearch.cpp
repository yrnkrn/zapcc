...
void InitHeaderSearch::AddUnmappedPath(const Twine &Path, IncludeDirGroup Group,
  ...
  if (Headers.CachingMode) {
    Path.toVector(MappedPathStorage);
    llvm::sys::fs::make_absolute(MappedPathStorage);
    llvm::sys::path::remove_dots(MappedPathStorage, true);
    MappedPathStr = MappedPathStorage;
  } else {
    MappedPathStr = Path.toStringRef(MappedPathStorage);
  }
  ...
  if (const DirectoryEntry *DE =
          FM.getDirectory(MappedPathStr, !Headers.CachingMode)) {
