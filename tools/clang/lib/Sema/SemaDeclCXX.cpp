...
bool Sema::MergeCXXFunctionDecl(FunctionDecl *New, FunctionDecl *Old,
  ...
  for (unsigned p = 0, NumParams = PrevForDefaultArgs
    ...
    if (OldParamHasDfl && NewParamHasDfl) {
      if (!isSameSourceLocation(OldParam->getLocStart(),
                                NewParam->getLocStart())) {
...
void Sema::CheckCompletedCXXClass(CXXRecordDecl *Record) {
  ...
  if (const RecordType *RT = dyn_cast<RecordType>(Record->getTypeForDecl())) {
    if (Context.NeedDestructor.count(RT)) {
      CXXDestructorDecl *Destructor = Record->getDestructor();
      if (!Destructor)
        Destructor = DeclareImplicitDestructor(Record);
      if (!Destructor->doesThisDeclarationHaveABody())
        MarkFunctionReferenced(Record->getLocation(), Destructor);
      // DefineImplicitDestructor(ClassDecl->getLocation(), Destructor);
    }
  }
}
  ...
  if (II) {
    ...
    if (!PrevNS) {
      UD = UsingDirectiveDecl::Create(Context, Parent,
                                      ...
                                      /* 'using' */ LBrace,
#else
                                      LangOpts.CachingMode ? SourceLocation() : LBrace,
#endif
                                      ...
                                      Namespc,
                                      /* Ancestor */ Parent);
      // UsingDirectiveDecl::Create sets namespace to OriginalNamespace,
      // but we need this one exactly.
      if (LangOpts.CachingMode)
        UD->setRawNominatedNamespace(Namespc);
      UD->setImplicit();
      Parent->addDecl(UD);
    }
  }
