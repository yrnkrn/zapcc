  ...
  if (!Invalid) {
    ...
    AlreadyInstantiating =
        !SemaRef.InstantiatingSpecializations
             ...
             .second;
    Decl::insertModifiedDecls(nullptr, Entity);
  }
}
...
Sema::InstantiateClass(SourceLocation PointOfInstantiation,
  ...
  for (auto *Member : Pattern->decls()) {
    ...
    Context.LiveDecls.insert(Member);
    if (NewMember) {
...
getPatternForClassTemplateSpecialization(
  ...
  if (Matched.size() >= 1) {
    ...
    Decl::insertModifiedDecls(Template, ClassTemplateSpec);
  } else {
    ...
    if (auto *Function = dyn_cast<FunctionDecl>(D)) {
      if (FunctionDecl *Pattern
        ...
        if (LangOpts.CachingMode && TSK == TSK_ExplicitInstantiationDefinition)
          Context.InstantiationChanges[Function] = {
              Old_TSK, MSInfo->getPointOfInstantiation(), PointOfInstantiation};
