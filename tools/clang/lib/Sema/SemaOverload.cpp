...
void Sema::AddOverloadedCallCandidates(UnresolvedLookupExpr *ULE,
  ...
  if (ULE->requiresADL())
    AddArgumentDependentLookupCandidates(ULE->getName(), ULE->getExprLoc(),
                                         ...
                                         CandidateSet, PartialOverloading);
  // For zapcc, we need to to handle test cases where due to caching result in
  // err_not_found_by_two_phase_lookup: the function should be declared prior to
  // the call site or in associated namespace. Such as: adl-visible.cpp (from
  // boost-icl) and unresolved-lookup-expr (from vcsn).
  if (ULE->requiresADL() && getLangOpts().CachingMode && CandidateSet.empty()) {
    LookupResult R(*this, ULE->getName(), ULE->getNameLoc(),
                   LookupOrdinaryName);
