...
Decl *TemplateDeclInstantiator::VisitFunctionDecl(FunctionDecl *D,
  ...
  if (auto *DGuide = dyn_cast<CXXDeductionGuideDecl>(D))
    ...
    if (FunctionDecl *Def = D->getDefinition())
      if (Def->isInlineSpecified())
        Function->setInlineSpecified(true);
...
static void setPointOfInstantiation(FunctionDecl *FD, SourceLocation SL) {
  if (MemberSpecializationInfo *MSInfo = FD->getMemberSpecializationInfo())
    MSInfo->setPointOfInstantiation(SL);
  if (FunctionTemplateSpecializationInfo *SpecInfo =
          FD->getTemplateSpecializationInfo())
    SpecInfo->setPointOfInstantiation(SL);
}

static void setPointOfInstantiation(VarDecl *VD, SourceLocation SL) {
  if (MemberSpecializationInfo *MSInfo = VD->getMemberSpecializationInfo())
    MSInfo->setPointOfInstantiation(SL);
  if (auto *VTSD = dyn_cast<VarTemplateSpecializationDecl>(VD))
    VTSD->setPointOfInstantiation(SL);
}

template <typename T>
bool missingInstantiationPattern(Sema &S, T *D, SourceLocation Loc) {
  if (!S.LangOpts.CachingMode)
    return false;
  T *Def;
  if (T *Pat = D->getTemplateInstantiationPattern())
    Def = Pat;
  else
    Def = D;
  Def = Def->getDefinition();
  if (Def && S.Context.LiveDecls.count(Def))
    return false;
  auto FileIDEntry = S.getSourceManager().getFileIDEntry(Loc);
  if (const FileEntry *FE = FileIDEntry.second) {
    S.CachedPendingInstantiations[FE][D] = Loc;
    setPointOfInstantiation(D, SourceLocation());
  }
  return true;
}
...
void Sema::PerformPendingInstantiations(bool LocalOnly) {
  while (!PendingLocalImplicitInstantiations.empty() ||
    ...
    if (FunctionDecl *Function = dyn_cast<FunctionDecl>(Inst.first)) {
      if (missingInstantiationPattern(*this, Function, Inst.second))
        continue;
    ...
    PrettyDeclStackTraceEntry CrashInfo(*this, Var, SourceLocation(),
                                        "instantiating variable definition");
    if (missingInstantiationPattern(*this, Var, Inst.second))
      continue;
