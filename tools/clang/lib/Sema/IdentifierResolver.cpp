...
void IdentifierResolver::EraseDecl(NamedDecl *D) {
  assert(D && "null param passed");
  DeclarationName Name = D->getDeclName();
  if (!Name)
    return;
  ...
  if (Kind == DeclarationName::NameKind::ObjCZeroArgSelector ||
      Kind == DeclarationName::NameKind::ObjCOneArgSelector ||
      Kind == DeclarationName::NameKind::ObjCMultiArgSelector ||
      Kind == DeclarationName::NameKind::CXXUsingDirective)
    return;
  void *Ptr = Name.getFETokenInfo<void>();
  if (!Ptr)
    return;
  NamedDecl *Prev = cast_or_null<NamedDecl>(D->getPreviousDecl());
  // In C mode, Sema::LookupName() uses IdResolver to find declarations.
  // Since Sema::PushOnScopeChains had removed the previous declaration
  // from IdResolver, we add it back. Required for duplicate-datalayout.
  // This logic seems too simplistic and may not be correct.
  if (Prev && !D->declarationReplaces(Prev))
    Prev = nullptr;
  if (isDeclPtr(Ptr)) {
    // assert(D == Ptr && "Didn't find this decl on its identifier's chain!");
    if (D == Ptr)
      Name.setFETokenInfo(Prev);
    return;
  }
  IdDeclInfo *IDI = toIdDeclInfo(Ptr);
  auto &Decls = IDI->Decls;
  auto I = std::remove_if(Decls.begin(), Decls.end(),
                          [&](NamedDecl *ND) { return D == ND; });
  Decls.erase(I, Decls.end());
}
