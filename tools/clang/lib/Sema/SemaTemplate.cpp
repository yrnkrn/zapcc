...
Sema::CheckVarTemplateId(VarTemplateDecl *Template, SourceLocation TemplateLoc,
          ...
          dyn_cast<VarTemplatePartialSpecializationDecl>(InstantiationPattern))
  {
    ...
    Decl::insertModifiedDecls(Template, Decl);
  }
...
Sema::ActOnClassTemplateSpecialization(Scope *S, unsigned TagSpec,
  ...
  if (TUK == TUK_Definition) {
    ...
    if (Def && SkipBody && (!hasVisibleDefinition(Def, &Hidden))) {
      ...
      TUK = TUK_Declaration;
    } else if (Def && SkipBody &&
        isSameSourceLocation(TemplateNameLoc, Def->getLocation())) {
      SkipBody->ShouldSkip = true;
      TUK = TUK_Declaration;
    } else if (Def) {
      SourceRange Range(TemplateNameLoc, RAngleLoc);
...
Sema::ActOnExplicitInstantiation(Scope *S,
  ...
  if (Def) {
    ...
    if (LangOpts.CachingMode && TSK == TSK_ExplicitInstantiationDefinition &&
        (Old_TSK == TSK_ImplicitInstantiation ||
         Old_TSK == TSK_ExplicitInstantiationDeclaration))
      // See Sema::InstantiateClassMembers.
      Context.InstantiationChanges[Def] = {Old_TSK, Def->getLocation(),
                                            TemplateLoc};
