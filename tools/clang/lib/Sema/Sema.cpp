...
void Sema::reset() {
  UnusedPrivateFields.clear();//401
  UnusedLocalTypedefNameCandidates.clear();//405
  // PureVirtualClassDiagSet 412 processed in Unloader::VisitCXXRecordDecl
  ParsingInitForAutoVars.clear();//416
  DeleteExprs.clear(); //466
  UnparsedDefaultArgLocs.clear();//881
  UndefinedButUsed.clear();//885
  DelayedTypos.clear();//2554
  VTableUses.clear();//4992
  VTablesUsed.clear();//4998
  CodeSynthesisContexts.clear();//6331
  InstantiatingSpecializations.clear();// 6856
  PendingInstantiations.clear(); // 7014
  Context.clearComments();
  if (LangOpts.CachingMode && CurContext)
    PopDeclContext();
}
...
Sema::~Sema() {
  ...
  if (SemaConsumer *SC = dyn_cast_or_null<SemaConsumer>(Consumer))
    SC->ForgetSema();
