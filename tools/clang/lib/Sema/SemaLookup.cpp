  ...
  for (; S; S = S->getParent()) {
    ...
    if (Ctx) {
      ...
      for (; Ctx && !Ctx->Equals(OuterCtx); Ctx = Ctx->getLookupParent()) {
        ...
        if (Found) {
          // Scan for possible function matches added after a cached using
          // namespace::foo statement. multi/using-ignored, 0mixed/iterator.cpp.
          // These will be found at the last namespace of existing matches.
          if (LangOpts.CachingMode) {
            llvm::SmallPtrSet<NamespaceDecl *, 4> LastNamespaces;
            llvm::SmallPtrSet<NamespaceDecl *, 4> FoundNamespaces;
            for (NamedDecl *ND : R)
              if (auto *USD = dyn_cast<UsingShadowDecl>(ND))
                if (auto *FD = dyn_cast<FunctionDecl>(USD->getTargetDecl()))
                  if (auto *NSD =
                          dyn_cast<NamespaceDecl>(FD->getDeclContext())) {
                    FoundNamespaces.insert(NSD);
                    NamespaceDecl *Last = NSD->getMostRecentDecl();
                    if (Last != NSD)
                      LastNamespaces.insert(Last);
                  }
            // noload_lookup finds everything in the Primary DeclContext
            // including the previous namespaces. We are interested in the
            // last namespace only.
            for (NamespaceDecl *NSD : LastNamespaces)
              if (!FoundNamespaces.count(NSD))
                for (NamedDecl *New : NSD->noload_lookup(Name)) {
                  if (New->getDeclContext() != NSD)
                    continue;
...
static NamedDecl *findAcceptableDecl(Sema &SemaRef, NamedDecl *D,
                                     unsigned IDNS, bool ForRedeclaration = true) {
#if 0
  ...
  for (auto RD : D->redecls()) {
    ...
    if (!ForRedeclaration && !SemaRef.wasIncluded(ND))
      continue;
    // See Unloader.cpp, VisitRedeclarable.
    if (SemaRef.LangOpts.CachingMode)
      if (TemplateDecl *TD = dyn_cast<TemplateDecl>(ND))
        if (TD->getFriendObjectKind() != Decl::FriendObjectKind::FOK_None)
          continue;
...
NamedDecl *LookupResult::getAcceptableDeclSlow(NamedDecl *D) const {
  if (auto *ND = dyn_cast<NamespaceDecl>(D)) {
    ...
    if (!isForRedeclaration() && !getSema().wasIncluded(Acceptable))
      Acceptable = findAcceptableDecl(getSema(), Key, isForRedeclaration());
    if (Acceptable)
      getSema().VisibleNamespaceCache.insert(std::make_pair(Key, Acceptable));
    return Acceptable;
  }

  return findAcceptableDecl(getSema(), D, IDNS, isForRedeclaration());
}

bool Sema::wasIncluded(const NamedDecl *ND) const {
  if (!getLangOpts().CachingMode)
    return true;
  if (!ND || ND->ignoreGeneratedDecl() || Context.LiveDecls.count(ND))
    return true;
  if (auto *CTSD = dyn_cast<ClassTemplateSpecializationDecl>(ND)) {
    if (CTSD->getTemplateSpecializationKind() == TSK_ImplicitInstantiation) {
      const CXXRecordDecl *CXXRD = CTSD->getTemplateInstantiationPattern();
      if (wasIncluded(CXXRD))
        return true;
    }
  } else if (auto *CTSD = dyn_cast<ClassTemplateSpecializationDecl>(
                 ND->getDeclContext())) {
    if (wasIncluded(CTSD))
      return true;
  }
  // FunctionTemplateDecl of Lambda expressions have invalid location.
  return ND->getLocStart().isInvalid();
}

static std::pair<const FileEntry *, unsigned>
getFileOffset(SourceManager &SM, SourceLocation Loc) {
  std::pair<FileID, unsigned> LocInfo;
  const FileEntry *FE;
  LocInfo = SM.getDecomposedLoc(Loc);
  FE = SM.getFileEntryForID(LocInfo.first);
  if (!FE) {
    LocInfo = SM.getDecomposedExpansionLoc(Loc);
    FE = SM.getFileEntryForID(LocInfo.first);
  }
  if (!FE) {
    LocInfo = SM.getDecomposedSpellingLoc(Loc);
    FE = SM.getFileEntryForID(LocInfo.first);
  }
  return std::make_pair(FE, LocInfo.second);
}

bool Sema::isSameSourceLocation(SourceLocation SL1, SourceLocation SL2) const {
  if (!LangOpts.CachingMode)
    return false;
  if (SL1.isInvalid() || SL2.isInvalid())
    return false;
  auto Loc1 = getFileOffset(SourceMgr, SL1);
  auto Loc2 = getFileOffset(SourceMgr, SL2);
