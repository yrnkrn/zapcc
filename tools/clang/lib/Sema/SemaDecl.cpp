...
NamedDecl *Sema::LazilyCreateBuiltin(IdentifierInfo *II, unsigned ID,
  ...
  // zapcc: create builtins w/o source information so they will not be unloaded.
...
bool Sema::checkVarDeclRedefinition(VarDecl *Old, VarDecl *New) {
  if (!hasVisibleDefinition(Old) &&
    ...
    if (!LangOpts.CachingMode ||
        !isSameSourceLocation(Old->getLocation(), New->getLocation())) {
...
Sema::CheckForFunctionRedefinition(FunctionDecl *FD,
  ...
  if (!FD->hasAttr<AliasAttr>() &&
      isSameSourceLocation(FD->getLocation(), Definition->getLocation())) {
    if (SkipBody)
      SkipBody->ShouldSkip = true;
    return;
  }
          ...
          if (TUK == TUK_Definition) {
            if (NamedDecl *Def = PrevTagDecl->getDefinition()) {
              ...
              if (SkipBody && !hasVisibleDefinition(Def, &Hidden)) {
                ...
                // Allow redefinition of tag decls at same source location.
                if (isSameSourceLocation(NameLoc, Def->getLocation())) {
                  if (SkipBody)
                    SkipBody->ShouldSkip = true;
                  return PrevTagDecl;
                } else {
...
Decl *Sema::ActOnEnumConstant(Scope *S, Decl *theEnumDecl, Decl *lastEnumConst,
  ...
  if (PrevDecl) {
    ...
    if (!isa<TagDecl>(PrevDecl) && isDeclInScope(PrevDecl, CurContext, S) &&
      ...
      // Allow redefinition of enum decls from same source location.
      if (isSameSourceLocation(IdLoc, PrevDecl->getLocation())) {
        TheEnumDecl->setInvalidDecl();
        return nullptr;
      // If not same location, let the new one win.
      } else if (!getLangOpts().CachingMode) {
