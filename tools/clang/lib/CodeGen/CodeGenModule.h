...
public:
  ...
  void addUsedGlobal(llvm::GlobalValue *GV, const Decl *D = nullptr);
  ...
  // GlobalValues associated with Decls and Types.
  // Will be made alive if decl is alive and erased if decl/type is erased.
  llvm::DenseMap<const Decl *, llvm::SmallVector<llvm::WeakTrackingVH, 4>>
      DeclGlobalValues;
  llvm::DenseMap<const Type *, llvm::SmallVector<llvm::WeakTrackingVH, 4>>
      TypeGlobalValues;
  // Typeinfos associated with Decls.
  // Will be made alive if gv is alive.
  llvm::DenseMap<llvm::Function *, llvm::SmallVector<llvm::WeakTrackingVH, 4>>
      FunctionTypeInfos;
  // GlobalDecls associated with NamedDecls.
  // FIXME: use SmallSet when it has begin() and end().
  llvm::DenseMap<const NamedDecl *, llvm::SmallVector<GlobalDecl, 4>>
      GlobalDecls;
  // Decls whose IR was erased by zapcc and should be codegenned again.
  llvm::SmallPtrSet<const NamedDecl *, 8> ErasedDeferredDeclsToEmit;
  void insertDeclGlobalValues(const Decl *D, llvm::GlobalValue *GV,
                              bool PreferFunctionBody = true);
  void getDeclGlobalValues(const Decl *D,
                           llvm::SmallVectorImpl<llvm::GlobalValue *> &GVs);
  void insertTypeGlobalValues(QualType QT, llvm::GlobalValue *GV);
  void insertFunctionTypeInfos(llvm::Function *F, llvm::Constant *C);
  MangleContext *getMangleContext();
  StringRef getMangledDeclName(GlobalDecl GD);
