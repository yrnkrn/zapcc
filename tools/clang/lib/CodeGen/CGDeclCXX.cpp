...
llvm::Constant *CodeGenFunction::createAtExitStub(const VarDecl &VD,
  ...
  llvm::Function *fn = CGM.CreateGlobalInitOrDestructFunction(ty, FnName.str(),
                                                              FI,
                                                              VD.getLocation());
  CGM.insertDeclGlobalValues(&VD, fn);
...
CodeGenModule::EmitCXXGlobalInitFunc() {
  ...
  bool CXXGlobalInitAlive = false;
  for (llvm::Function *CXXGlobalInit : CXXGlobalInits)
    if (CXXGlobalInit && CXXGlobalInit->isAlive()) {
      CXXGlobalInitAlive = true;
      break;
    }
  if (!CXXGlobalInitAlive)
    return;
