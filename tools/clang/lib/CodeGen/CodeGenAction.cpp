...
namespace clang {
  ...
  public:
    BackendConsumer(BackendAction Action, DiagnosticsEngine &Diags,
                    ...
                    CompilerInstance &CI, CoverageSourceInfo *CoverageInfo = nullptr)
        : Diags(Diags), Action(Action), HeaderSearchOpts(HeaderSearchOpts),
          CodeGenOpts(CodeGenOpts), TargetOpts(TargetOpts), LangOpts(LangOpts),
          AsmOutStream(std::move(OS)), Context(nullptr),
          LLVMIRGeneration("irgen", "LLVM IR Generation Time"),
          LLVMIRGenerationRefCount(0),
          CachingMode(CI.getLangOpts().CachingMode),
          Gen(CI.CodeGen
                  ? CI.CodeGen.get() :
                  CreateLLVMCodeGen(Diags, InFile, HeaderSearchOpts, PPOpts,
                                      CodeGenOpts, C, CoverageInfo)),
          CI(CI),
          LinkModules(std::move(LinkModules)) {
      if (CachingMode) {
        assert(Action == Backend_EmitObj || Action == Backend_EmitBC ||
               Action == Backend_EmitLL);
        Gen->GetModule()->setModuleIdentifier(InFile);
        Gen->GetModule()->setSourceFileName(InFile);
        Gen->GetModule()->CachingMode = true;
      }
    ...
    void Initialize(ASTContext &Ctx) override {
      ...
      if (!CachingMode || !CI.CodeGen)
      Gen->Initialize(Ctx);

      if (CachingMode && !CI.CodeGen)
        CI.CodeGen.reset(Gen.get());
    ...
    void HandleTranslationUnit(ASTContext &C) override {
      if (CachingMode && Diags.hasErrorOccurred())
        return;
      {
	...
	IRGenFinished = true;
      ...
      if (CachingMode)
        CI.CachingEmitObj(Action, AsmOutStream.get());
      else
...
void CodeGenAction::EndSourceFileAction() {
  ...
  if (CI.getLangOpts().CachingMode) {
    if (!CI.VMContext.get()) {
      CI.VMContext.reset(VMContext);
      OwnsVMContext = false;
    }
  } else
...
CodeGenAction::CreateASTConsumer(CompilerInstance &CI, StringRef InFile) {
  ...
  llvm::LLVMContext &Ctx =
      CI.VMContext.get() ? *CI.VMContext.get() : *VMContext;

  std::unique_ptr<BackendConsumer> Result(new BackendConsumer(
      BA, CI.getDiagnostics(), CI.getHeaderSearchOpts(),
      CI.getPreprocessorOpts(), CI.getCodeGenOpts(), CI.getTargetOpts(),
      CI.getLangOpts(), CI.getFrontendOpts().ShowTimers, InFile,
      std::move(LinkModules), std::move(OS), Ctx, CI, CoverageInfo));
