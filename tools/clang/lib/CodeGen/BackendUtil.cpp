...
public:
  ...
  bool AddEmitPasses(legacy::PassManagerBase &CodeGenPasses, BackendAction Action,
...
bool EmitAssemblyHelper::AddEmitPasses(legacy::PassManagerBase &CodeGenPasses,
...
CachingPassHelper::CachingPassHelper(
    DiagnosticsEngine &Diags, const HeaderSearchOptions &HeaderSearchOpts,
    const CodeGenOptions &CGOpts, const clang::TargetOptions &TOpts,
    const LangOptions &LOpts, llvm::Module *M) {
  EAH = new EmitAssemblyHelper(Diags, HeaderSearchOpts, CGOpts, TOpts, LOpts, M);
  PerModulePasses = new llvm::legacy::PassManager;
  PerFunctionPasses = new llvm::legacy::FunctionPassManager(M);
}

void CachingPassHelper::CreatePasses() {
  EAH->CreatePasses(*PerModulePasses, *PerFunctionPasses);
}

bool CachingPassHelper::AddEmitPasses(legacy::PassManagerBase &CodeGenPasses,
                                      BackendAction Action,
                                      raw_pwrite_stream &OS) {
  return EAH->AddEmitPasses(CodeGenPasses, Action, OS);
}
...
TargetMachine *CachingPassHelper::CreateTargetMachine(bool MustCreateTM) {
  if (!EAH->TM)
    EAH->CreateTargetMachine(MustCreateTM);
  return EAH->TM.get();
}
