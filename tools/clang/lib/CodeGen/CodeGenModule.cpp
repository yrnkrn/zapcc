...
void CodeGenModule::applyReplacements() {
  for (auto &I : Replacements) {
    ...
    if (const Decl *D = reinterpret_cast<const Decl *>(OldF->getDecl())) {
      // NewF should not be dependent on Decl. Remove OldF before it is replaced
      // by NewF. OldF will be anyhow removed from its parent module below.
      auto I = DeclGlobalValues.find(D);
      assert(I != DeclGlobalValues.end());
      auto &OldVec = I->second;
      OldVec.erase(std::remove(OldVec.begin(), OldVec.end(), OldF),
                   OldVec.end());
    }
...
void CodeGenModule::checkAliases() {
  ...
  for (const GlobalDecl &GD : Aliases) {
    ...
    llvm::GlobalValue *Entry = GetGlobalValue(MangledName);
    if (!Entry || !isa<llvm::GlobalIndirectSymbol>(Entry))
      continue;
...
void CodeGenModule::insertDeclGlobalValues(const Decl *D, llvm::GlobalValue *GV,
                                           bool PreferFunctionBody) {
  if (!LangOpts.CachingMode)
    return;
  assert(GV);
#if 0
  if (const Decl *CurD = reinterpret_cast<const Decl *>(GV->getDecl())) {
    if (D == CurD)
      return;
    // Remove GV from old Decl.
    auto I = DeclGlobalValues.find(CurD);
    assert(I != DeclGlobalValues.end());
    auto &CurVec = I->second;
    CurVec.erase(std::remove(CurVec.begin(), CurVec.end(), GV), CurVec.end());
  }
#endif
  if (D) {
    if (PreferFunctionBody && !GV->isDeclaration() && isa<llvm::Function>(GV))
      if (const FunctionDecl *FD = dyn_cast<FunctionDecl>(D)) {
        FD->hasBody(FD);
        D = FD;
      }
    auto &GVs = DeclGlobalValues[D];
    if (std::any_of(GVs.begin(), GVs.end(),
                    [=](llvm::Value *V) { return V == GV; }))
      return;
    GVs.emplace_back(GV);
  }
  GV->setDecl(D);
...
void CodeGenModule::getDeclGlobalValues(
    const Decl *D, llvm::SmallVectorImpl<llvm::GlobalValue *> &GVs) {
  auto I = DeclGlobalValues.find(D);
  if (I == DeclGlobalValues.end())
    return;
  for (llvm::Value *V : I->second)
    if (llvm::GlobalValue *GV = llvm::GlobalValue::fromValue(V))
      GVs.push_back(GV);
}

void CodeGenModule::insertTypeGlobalValues(QualType QT, llvm::GlobalValue *GV) {
  if (!LangOpts.CachingMode)
    return;
  assert(GV);
  auto &GVs = TypeGlobalValues[&*QT];
  if (std::none_of(GVs.begin(), GVs.end(),
                   [=](llvm::Value *V) { return V == GV; }))
    GVs.emplace_back(GV);
}

void CodeGenModule::insertFunctionTypeInfos(llvm::Function *F,
                                            llvm::Constant *C) {
  if (!LangOpts.CachingMode)
    return;
  assert(F);
  ...
  if (llvm::GlobalVariable *GV = dyn_cast<llvm::GlobalVariable>((C))) {
    auto &TypeInfos = FunctionTypeInfos[F];
    if (std::any_of(TypeInfos.begin(), TypeInfos.end(),
                    [=](llvm::Value *V) { return V == GV; }))
      return;
    TypeInfos.push_back(GV);
  }
  for (llvm::Use &U : C->operands())
    if (llvm::Constant *Op = dyn_cast<llvm::Constant>(U))
      insertFunctionTypeInfos(F, Op);
}

MangleContext *CodeGenModule::getMangleContext() {
  return &ABI->getMangleContext();
}

StringRef CodeGenModule::getMangledDeclName(GlobalDecl GD) {
  auto I = MangledDeclNames.find(GD.getCanonicalDecl());
  if (I != MangledDeclNames.end())
    return I->second;
  else
    return StringRef();
}
...
StringRef CodeGenModule::getMangledName(GlobalDecl GD) {
  ...
  if (LangOpts.CachingMode) {
    auto &GDs = GlobalDecls[cast<NamedDecl>(GD.getDecl())];
    if (!std::any_of(GDs.begin(), GDs.end(),
                     [=](GlobalDecl gd) { return gd == CanonicalGD; }))
      GDs.push_back(CanonicalGD);
  }
  ...
  // Unloader replaces with null GlobalDecl. If such is found, do overwrite it.
  if (!Result.second)
    if (!Result.first->second.getDecl())
      Manglings[getMangledNameImpl(*this, GD, ND)] = GD;
...
void CodeGenModule::EmitCtorList(CtorList &Fns, const char *GlobalName) {
  ...
  for (const auto &I : Fns) {
    if (!cast<llvm::Function>(I.Initializer)->isAliveDefinition())
      continue;
...
void CodeGenModule::addUsedGlobal(llvm::GlobalValue *GV, const Decl *D) {
...
void CodeGenModule::emitLLVMUsed() {
  decltype(LLVMUsed) List;
  for (auto I : LLVMUsed)
    if (llvm::GlobalValue *GV = dyn_cast<llvm::GlobalValue>(&*I))
      if (GV->isAlive())
        List.push_back(GV);

  emitUsed(*this, "llvm.used", List);
...
void CodeGenModule::EmitGlobalAnnotations() {
  decltype(Annotations) AliveAnnotations;
  if (TheModule.CachingMode) {
    for (llvm::Constant *C : Annotations) {
      llvm::ConstantStruct *CS = cast<llvm::ConstantStruct>(C);
      llvm::Constant *BitCast = CS->getOperand(0);
      llvm::GlobalValue *GV = cast<llvm::GlobalValue>(BitCast->stripPointerCasts());
      if (!GV->isAliveDefinition())
        continue;
      AliveAnnotations.push_back(C);
    }
    if (!AliveAnnotations.size())
      return;
    AliveAnnotations.swap(Annotations);
  }
...
CodeGenModule::GetOrCreateLLVMGlobal(StringRef MangledName,
  ...
  if (Entry) {
    if (LangOpts.CachingMode) {
      if (D) {
        insertDeclGlobalValues(D, Entry);
        if (D->getTLSKind()) {
...
static void replaceUsesOfNonProtoConstant(llvm::Constant *old,
  ...
  for (llvm::Value::use_iterator ui = old->use_begin(), ue = old->use_end();
    ...
    if (llvm::Instruction *Ins = dyn_cast<llvm::Instruction>(user))
      Ins->getParent()->getParent()->invalidateMachineFunction();
...
#if 0
  // Relevant for xcore target only, linear time in number of functions.
