...
public:
  ...
  void eraseUserDecls(
    ...
    for (const CXXRecordDecl *CXXRD : UserCXXRecords)
      VTables.erase(cast<CXXRecordDecl>(CXXRD));
  }
  void eraseUserConstants(
      llvm::SetVector<llvm::Constant *> &ConstantsToEraseCompletely) override {
    for (auto I = VTables.begin(), E = VTables.end(); I != E;) {
      auto J = I;
      J++;
      if (ConstantsToEraseCompletely.count(I->second))
        VTables.erase(I);
      I = J;
    }
  }
...
void ItaniumCXXABI::EmitGuardedInit(CodeGenFunction &CGF,
  ...
  if (!guard) {
    ...
    CGM.insertDeclGlobalValues(&D, guard);
...
ItaniumCXXABI::getOrCreateThreadLocalWrapper(const VarDecl *VD,
  ...
  CGM.insertDeclGlobalValues(VD, Wrapper);
...
void ItaniumCXXABI::EmitThreadLocalInitFuncs(
  ...
  for (const VarDecl *VD : CXXThreadLocals) {
    ...
    if (Init)
    {
      CGM.insertDeclGlobalValues(VD, Init);
...
static RecordDecl *getRecord(QualType QT) {
#if 0
  while (true) {
    QualType Pointee = QT->getPointeeType();
    if (Pointee.isNull())
      break;
    QT = Pointee.getCanonicalType();
  }
#endif
...
llvm::GlobalVariable *ItaniumRTTIBuilder::GetAddrOfTypeName(
  ...
  CGM.insertTypeGlobalValues(Ty, GV);
  if (RecordDecl *RD = getRecord(Ty))
...
static void emitConstructorDestructorAlias(CodeGenModule &CGM,
  ...
  if (Entry)
    Entry->setAliveDefinition();
  if (Entry && !Entry->isDeclaration())
    return;

  auto *Aliasee = cast<llvm::GlobalValue>(CGM.GetAddrOfGlobal(TargetDecl));
  // We are creating new alias, remove any old replacement which may be
  // incompatible as zapcc/multi/implicit-explicit-template/implicit-template.
  CGM.Replacements.erase(MangledName);
  CGM.Replacements.erase(Aliasee->getName());

  // Create the alias with no name.
  auto *Alias = llvm::GlobalAlias::create(Linkage, "", Aliasee);
  CGM.insertDeclGlobalValues(TargetDecl.getDecl(), Alias);
...
void ItaniumCXXABI::emitCXXStructor(const CXXMethodDecl *MD,
  ...
  CGM.insertDeclGlobalValues(MD, Fn);
