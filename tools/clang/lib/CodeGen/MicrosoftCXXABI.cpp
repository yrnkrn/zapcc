...
public:
  ...
  void eraseUserDecls(
      llvm::SmallPtrSetImpl<const CXXRecordDecl *> &UserCXXRecords) override {

    // DeferredVFTables
    for (const CXXRecordDecl *CXXRD : UserCXXRecords) {
      DeferredVFTables.erase(CXXRD);
      VBTablesMap.erase(CXXRD);
    }

    // VTablesMap
    for (auto I = VTablesMap.begin(), E = VTablesMap.end(); I != E;) {
      auto J = I;
      ++J;
      const CXXRecordDecl *CXXRD = I->first.first;
      if (UserCXXRecords.count(CXXRD))
        VTablesMap.erase(I);
      I = J;
    }

    // VFTablesMap
    for (auto I = VFTablesMap.begin(), E = VFTablesMap.end(); I != E;) {
      auto J = I;
      ++J;
      const CXXRecordDecl *CXXRD = I->first.first;
      if (UserCXXRecords.count(CXXRD))
  ...
  void eraseUserConstants(
      llvm::SetVector<llvm::Constant *> &ConstantsToEraseCompletely) override {

    // VBTablesMap
    for (auto I = VBTablesMap.begin(), E = VBTablesMap.end(); I != E;) {
      auto J = I;
      ++J;
      auto &GVs = I->second.Globals;
      if (std::any_of(GVs.begin(), GVs.end(), [&](llvm::GlobalVariable *GV) {
            return ConstantsToEraseCompletely.count(GV);
          }))
        VBTablesMap.erase(I);
      I = J;
    }
    ...
    for (auto I = VTablesMap.begin(), E = VTablesMap.end(); I != E;) {
      auto J = I;
      ++J;
      if (llvm::GlobalVariable *GV = I->second)
        if (ConstantsToEraseCompletely.count(GV))
          VTablesMap.erase(I);
      I = J;
    }
    ...
    for (auto I = VFTablesMap.begin(), E = VFTablesMap.end(); I != E;) {
      auto J = I;
      ++J;
      if (llvm::GlobalValue *GV = I->second)
        if (ConstantsToEraseCompletely.count(GV))
          VFTablesMap.erase(I);
      I = J;
    }
  ...
  getAddrOfVirtualDisplacementMap(const CXXRecordDecl *SrcRD,
    ...
    CGM.insertDeclGlobalValues(DstRD, VDispMap);
    return VDispMap;
  }
...
void MicrosoftCXXABI::emitVTableDefinitions(CodeGenVTables &CGVT,
  ...
  for (const std::unique_ptr<VPtrInfo>& Info : VFPtrs) {
    ...
    if (CGM.getLangOpts().CachingMode)
      VTable->setAliveDefinition();
    else
...
llvm::GlobalVariable *MicrosoftCXXABI::getAddrOfVTable(const CXXRecordDecl *RD,
  ...
  if (VTableAliasIsRequred) {
    ...
    VFTable = llvm::GlobalAlias::create(CGM.Int8PtrTy,
                                        ...
                                        &CGM.getModule());
    CGM.insertDeclGlobalValues(RD, VFTable);
...
MicrosoftCXXABI::getAddrOfVBTable(const VPtrInfo &VBT, const CXXRecordDecl *RD,
  ...
  CGM.insertDeclGlobalValues(RD, GV);
...
void MicrosoftCXXABI::EmitGuardedInit(CodeGenFunction &CGF, const VarDecl &D,
  ...
  if (!GuardVar) {
    ...
    if (GI && !HasPerVariableGuard)
      GI->Guard = GuardVar;
    CGM.setStaticLocalDeclGuardAddress(&D, GuardVar);
  }
...
llvm::GlobalVariable *MSRTTIBuilder::getClassHierarchyDescriptor() {
  ...
  auto CHD = new llvm::GlobalVariable(Module, Type, /*Constant=*/true, Linkage,
                                      /*Initializer=*/nullptr,
                                      MangledName);
  CGM.insertDeclGlobalValues(RD, CHD);
...
MSRTTIBuilder::getBaseClassArray(SmallVectorImpl<MSRTTIClass> &Classes) {
  ...
  auto *BCA =
      new llvm::GlobalVariable(Module, ArrType,
                               ...
                               /*Initializer=*/nullptr, MangledName);
  CGM.insertDeclGlobalValues(RD, BCA);
...
MSRTTIBuilder::getBaseClassDescriptor(const MSRTTIClass &Class) {
  ...
  auto BCD =
      new llvm::GlobalVariable(Module, Type, /*Constant=*/true, Linkage,
                               /*Initializer=*/nullptr, MangledName);
  CGM.insertDeclGlobalValues(Class.RD, BCD);
...
MSRTTIBuilder::getCompleteObjectLocator(const VPtrInfo &Info) {
  ...
  auto COL = new llvm::GlobalVariable(Module, Type, /*Constant=*/true, Linkage,
    /*Initializer=*/nullptr, MangledName);
  CGM.insertDeclGlobalValues(RD, COL);
...
llvm::Constant *MicrosoftCXXABI::getAddrOfRTTIDescriptor(QualType Type) {
  ...
  auto *Var = new llvm::GlobalVariable(
      ...
      MangledName);
  CGM.insertTypeGlobalValues(Type, Var);
...
llvm::GlobalVariable *MicrosoftCXXABI::getCatchableTypeArray(QualType T) {
  ...
  CGM.insertTypeGlobalValues(T, CTA);
...
llvm::GlobalVariable *MicrosoftCXXABI::getThrowInfo(QualType T) {
  ...
  auto *GV = new llvm::GlobalVariable(
      ...
      llvm::ConstantStruct::get(TIType, Fields), MangledName);
  CGM.insertTypeGlobalValues(T, GV);
  GV->setUnnamedAddr(llvm::GlobalValue::UnnamedAddr::Global);
  GV->setSection(".xdata");
  if (GV->isWeakForLinker())
    GV->setComdat(CGM.getModule().getOrInsertComdat(GV->getName()));
