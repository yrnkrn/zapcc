...
static std::string getStaticDeclName(CodeGenModule &CGM, const VarDecl &D) {
  ...
  if (CGM.getLangOpts().CachingMode) {
    ...
    auto &GDs = CGM.GlobalDecls[&D];
    if (!std::any_of(GDs.begin(), GDs.end(),
                     [=](GlobalDecl gd) { return gd == Canonical; }))
      GDs.push_back(Canonical);
    auto Result = CGM.Manglings.insert(std::make_pair(ContextName, GD));
    CGM.MangledDeclNames[Canonical] = Result.first->first();
  }
  return ContextName;
}
...
void CodeGenFunction::EmitAutoVarInit(const AutoVarEmission &emission) {
  ...
  if (shouldUseMemSetPlusStoresToInitialize(constant,
    ...
    CGM.insertDeclGlobalValues(&D, GV);
