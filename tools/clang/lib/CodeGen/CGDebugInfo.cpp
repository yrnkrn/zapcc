...
void CGDebugInfo::CreateCompileUnit() {
  ...
  if (TheCU) {
    ...
    llvm::NamedMDNode *NMD = M.getOrInsertNamedMetadata("llvm.dbg.cu");
    if (!NMD->getNumOperands())
      NMD->addOperand(TheCU);
    TheCU->replaceSourceLanguage(LangTag);
    TheCU->replaceFilename(remapDIPath(MainFileName),
                           remapDIPath(getCurrentDirname()));
  } else
...
void CGDebugInfo::finalize() {
  ...
  for (auto p : ReplaceMap) {
    ...
    // We may not find the replacement type left from a previous run
    // since this run may not need it anymore and so CloneModuleActive
    // did not copy it. In such case, it's OK to ignore and continue.
    if (it == TypeCache.end())
      continue;
    assert(it->second);
  ...
  DBuilder.finalize();
  ReplaceMap.clear();
  FwdDeclReplaceMap.clear();
  LexicalBlockStack.clear();
  FnBeginRegionCount.clear();
}
