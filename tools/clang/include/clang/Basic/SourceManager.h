...
public:
  ...
  std::pair<FileID, const FileEntry *> getFileIDEntry(SourceLocation SL) {
    if (SL.isInvalid())
      return {FileID(), nullptr};
    SL = getFileLoc(SL);
    FileID FID = getFileID(SL);
    return {FID, getFileEntryForID(FID)};
  }
  ...
  SrcMgr::ContentCache *eraseFE(const FileEntry *FE) {
    if (OverriddenFilesInfo) {
      OverriddenFilesInfo->OverriddenFiles.erase(FE);
      OverriddenFilesInfo->OverriddenFilesWithBuffer.erase(FE);
    }
    auto I = FileInfos.find(FE);
    assert(I != FileInfos.end());
    FileInfos.erase(I);
    return I->second;
  }
  SrcMgr::ContentCache *getContentCache(const FileEntry *FE) {
    auto I = FileInfos.find(FE);
    assert(I != FileInfos.end());
    return I->second;
  }
