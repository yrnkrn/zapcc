...
public:
  llvm::DenseMap<const IdentifierInfo *, const FileEntry *> IncludeGuards;
  // Second (includer) depends on first (includee).
  llvm::SmallVector<std::pair<const FileEntry *, const FileEntry *>, 8> NewFEs;
#ifndef NDEBUG
  llvm::SmallPtrSet<const FileEntry *, 8> ParsedFEs;
#endif
  llvm::SetVector<const FileEntry *> VisibleFEs;
  typedef llvm::SmallVector<std::pair<IdentifierInfo *, MacroDirective *>, 8>
      MacrosVector;
  llvm::DenseMap<const FileEntry *, MacrosVector> GeneratedMacros;
  MacrosVector NeedMacros;
  MacrosVector GeneratedMacrosCommandLine;
  typedef llvm::DenseMap<IdentifierInfo *, StringRef> IdentifierValues;
  // Identifiers set from commands line or main file.
  IdentifierValues CommandLineMainIdentifiersIdentifiers;
  llvm::DenseMap<const FileEntry *, IdentifierValues> FilesIdentifiersValues;
  class CachingIncludeManager {
    Preprocessor &PP;
    // Included from index.
    typedef llvm::DenseMap<const FileEntry *,
                           llvm::SmallVector<const FileEntry *, 8>>
        IncludeesMapType;
    IncludeesMapType IncludeesMap;
    llvm::DenseMap<const FileEntry *, SourceLocation> LocForStartOfFile;
    void doCallbacks(const FileEntry *Includee);

  public:
    // Newly-included and visible FileEntries in this compilation.
    llvm::SmallPtrSet<const FileEntry *, 8> Includes;

    CachingIncludeManager(Preprocessor &PP) : PP(PP) {}
    void include(const FileEntry *Includee);
    void include(const FileEntry *Includee, const FileEntry *Includer);
    std::function<void(const FileEntry *)> IncludedCallBack;
    bool wasIncluded(SourceLocation SL) const;
    void erase(const FileEntry *FE);
    void setLocForStartOfFile(const FileEntry *FE, SourceLocation SL) {
      LocForStartOfFile[FE] = SL;
    }
  };
  CachingIncludeManager IncludeMgr{*this};
  void macroDefined(IdentifierInfo *II, MacroDirective *MD);
  void macroIdentifierUsed(IdentifierInfo *II);
  void macroCheckChanged(const FileEntry *Includee);
  bool wasIncluded(const IdentifierInfo *II, MacroDirective *MD) const;

  bool InsertedIgnorePragmas;
  bool InsertedVisibleFEs;
  void reset();
