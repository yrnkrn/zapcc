...
public:
  void clearComments() {
    Comments.clear();
    RedeclComments.clear();
    ParsedComments.clear();
  }

  struct InstantiationChange {
    TemplateSpecializationKind Old_TSK;
    SourceLocation OldLoc;
    SourceLocation NewLoc;
  };
  // Destructor was needed but not available for Delete operator.
  llvm::SmallPtrSet<const RecordType *, 4> NeedDestructor;
  llvm::DenseMap<const NamedDecl *, InstantiationChange> InstantiationChanges;
  mutable llvm::SmallPtrSet<const Decl *, 1> LiveDecls;
  // Set to avoid duplicates Sema::InstantiatingTemplate::InstantiatingTemplate.
  mutable llvm::SmallVector<Decl *, 8> NewDecls;
  // { Dependee, Dependent }
  mutable llvm::SmallVector<std::pair<Decl *, Decl *>, 8> ModifiedDecls;
  mutable llvm::SmallVector<Type *, 32> NewTypes;
  mutable llvm::SmallVector<NestedNameSpecifier *, 32> NewNestedNameSpecifiers;
  // Enable removing unloaded decls from OE without creating dependencies.
  mutable llvm::DenseMap<const NamedDecl *,
                         llvm::SmallVector<OverloadExpr *, 2>>
      OverloadExprs;
