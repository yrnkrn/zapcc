...
public:
  void eraseTagDecl(const TagDecl *TD) { AnonStructIds.erase(TD); }
  void eraseBlockDecl(const BlockDecl *BD) {
    GlobalBlockIds.erase(BD);
    LocalBlockIds.erase(BD);
  }
  virtual void eraseNamedDecl(const NamedDecl *ND) = 0;
  virtual void eraseCXXRecordDecl(const CXXRecordDecl *CXXRD) = 0;
