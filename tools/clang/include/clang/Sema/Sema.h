...
public:
  ...
  Scope *TUScope;
  std::unique_ptr<Scope> CachedTUScope;
  bool wasIncluded(const NamedDecl *ND) const;
  bool isSameSourceLocation(SourceLocation SL1, SourceLocation SL2) const;
...
public:
  ...
  // zapcc avoid duplicate missing instatiation record in every source file.
  llvm::DenseMap<const FileEntry *, llvm::DenseMap<ValueDecl *, SourceLocation>>
      CachedPendingInstantiations;
